...
folder_path = r'C:\Users\oohus\OneDrive - Trøndelag fylkeskommune\Dokumenter\Skole\VG3\IT2\Theater'
file_to_search = (epost + '.json')

found_file_path = search_file(folder_path, file_to_search)
if found_file_path:
    #Hent ut navn, etternavn, tlf fra JSON filen til bestemt bruker
    ...
    if forestilling.lower() == "vildanden":
        if antall_billetter > 0:
            ...
            if antall_billetter <= sheet["D" + str(dato + 1)].value:
                ...
                print(f"Antall plasser igjen: {sheet['D' + str(dato + 1)].value}")
                break
            else:
                svar = input(
    ...
    if forestilling.lower() == "de elendige":
        if antall_billetter > 0:
            ...
            if antall_billetter <= sheet["B" + str(dato + 1)].value:
                ...
                print(f"Antall plasser igjen: {sheet['B' + str(dato + 1)].value}")
                break
            else:
                svar = input(
                    f"De er ingen ledige forestillinger for {forestilling} for {dato}.februar. Vil du velge et annet tidspunkt? (J/N) "
...
print(f"Din bestilling: Du ({fornavn} {etternavn}) har bestillt {honnor} honørbilletter, {voksen} voksenbilletter, {student} studentbilletter, {barn} barnebilletter. Pris: {Billett(forestilling, dato, honnor, voksen, student, barn).beregn_pris()} kr. Din mail addresse er {epost}. Tlf: {tlf} ")
