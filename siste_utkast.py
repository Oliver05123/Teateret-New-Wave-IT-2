...
class Billett:
    ...
    def eksporter_bestilling(
        ...
        try:
            with open(filnavn + '.json', 'r') as fil:
                json_data = json.load(fil)
                bestillingsnummer = json_data.get('bestillingsnummer', 1) + 1

        except FileNotFoundError:
            bestillingsnummer = 2

        json_data = {'bestillingsnummer': bestillingsnummer}

        with open(filnavn + '.json', 'w') as fil:
        ...
        sheet["A" + str(bestillingsnummer)] = self.forestilling
        sheet["B" + str(bestillingsnummer)] = self.honnor
        sheet["C" + str(bestillingsnummer)] = self.student
        sheet["D" + str(bestillingsnummer)] = self.barn
        sheet["E" + str(bestillingsnummer)] = self.voksen
        sheet["F" + str(bestillingsnummer)] = self.beregn_pris()
        sheet["g" + str(bestillingsnummer)] = self.dato
        workbook.save(filnavn + ".xlsx")
        sheet["A" + str(bestillingsnummer)].protection = openpyxl.styles.Protection(locked=True)
        sheet["B" + str(bestillingsnummer)].protection = openpyxl.styles.Protection(locked=True)
        sheet["C" + str(bestillingsnummer)].protection = openpyxl.styles.Protection(locked=True)
        sheet["D" + str(bestillingsnummer)].protection = openpyxl.styles.Protection(locked=True)
        sheet["E" + str(bestillingsnummer)].protection = openpyxl.styles.Protection(locked=True)
        sheet["F" + str(bestillingsnummer)].protection = openpyxl.styles.Protection(locked=True)
        sheet["g" + str(bestillingsnummer)].protection = openpyxl.styles.Protection(locked=True)
    ...
    def vis_kvittering(
        ...
        temp_var = True
        if temp_var == True:#len(forestillinger) >= 2:
            while True:
                svar = input(
